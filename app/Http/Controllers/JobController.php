...
class JobController extends Controller
{
    ...
    public function apply(Request $request)
    {
        ...
        $pvi = Pvi::where('status', 1)->count();
        $pvi_answer = PviAnswer::where('candidate_id', $candidate->id)->count();
        
        if ($pvi != $pvi_answer) {
            return back()->with('message', 'PVI belum lengkap! harap lengkapi PVI di halaman Profil!');
        } else {
            $status = 0;
